...
def find_frame_candidate(roi_gray, sensitivity='normal', debug_prefix=''):
    ...
    for cnt in contours:
        ...
        if DEBUG_MODE and debug_vis is not None:
            cv2.rectangle(debug_vis, (x, y), (x+w, y+h), (0, 255, 0), 2)  # Yeşil: geçti
    ...
    if candidates:
        ...
        return candidates[0][1], debug_info  # En iyi konturu döndür
...
def process_image(img_path, face_cascade):
    ...
    for face_idx, (fx, fy, fw, fh) in enumerate(faces):
        ...
        roi_y_start = int(fy + fh * 0.15)  # Biraz yukarıdan başla
        roi_y_end = int(fy + fh * 0.65)    # %55'ten %65'e çıkarıldı (çerçeve altını yakalamak için)
