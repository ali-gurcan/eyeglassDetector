...
def clear_output_dir():
    ...
    # Recreate the required YOLO subdirectories
    os.makedirs(os.path.join(OUTPUT_FOLDER, 'images'), exist_ok=True)
    os.makedirs(os.path.join(OUTPUT_FOLDER, 'labels'), exist_ok=True)
...
def process_image(img_path, face_cascade, eye_cascade):
    ...
    # 3. CONSOLIDATE RESULTS & YOLO EXPORT
    yolo_lines = []
    h, w = img.shape[:2]
    ...
    for face_idx, sides in eye_results.items():
        for side, res in sides.items():
            ...
            if res['cnt'] is not None:
                ...
                # Class 1 = glass
                yolo_lines.append("1 " + " ".join(yolo_pts))
    ...
    if total_hulls > 0:
        # Resmi "images" klasörüne kopyala/kaydet
        out_img_path = Path(OUTPUT_FOLDER) / "images" / filename
        cv2.imwrite(str(out_img_path), img)
        
        # Etiket dosyasını "labels" klasörüne kaydet
        out_txt_path = Path(OUTPUT_FOLDER) / "labels" / f"{input_path.stem}.txt"
        with open(out_txt_path, 'w') as f:
            f.write("\n".join(yolo_lines))
            
        print(f"[ETIKETLENDI] {filename} -> {total_hulls} cam YOLO formatında kaydedildi.")
    else:
        print(f"[UYARI] {filename} -> Bulunamadı (Atlandı).")
